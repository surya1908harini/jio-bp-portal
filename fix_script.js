const fs = require('fs');
const path = 'C:/Users/M-tech/.gemini/antigravity/scratch/jio-bp-portal/frontend/src/pages/BudgetPage.jsx';
let code = fs.readFileSync(path, 'utf8');

const regex = /const totalRemainingBudget = totalFoBudget - totalConsumedBudget([\s\S]*?)<div className=\"flex-1 min-w-0 pr-2\">/m;

const replacement = const totalRemainingBudget = totalFoBudget - totalConsumedBudget
  ...
  return (
    <div className="space-y-5">
      ...
      <ModuleHeader
        ...
        stats={[
          { icon: FileText, label: 'Work Orders', value: fyRecords.length, sub: activeFy === 'overall' ? 'All FY' : \FY \\, color: 'orange' },
                  ...
                  <div key={b.id} className="glass-card p-4 relative group/card hover:-translate-y-1 transition-all duration-300 flex flex-col h-full border border-slate-700/50 hover:border-jio-blue-500/50 shadow-lg hover:shadow-jio-blue-500/10" onClick={() => setSelectedRow(b)}>
                    {isAdmin && (
                      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover/card:opacity-100 transition-opacity bg-slate-900/80 backdrop-blur-md p-1 rounded-lg border border-slate-700 z-10">
                        <button onClick={(e) => { e.stopPropagation(); openEdit(b) }} className="p-1 hover:text-white text-slate-400"><Pencil size={12} /></button>
                        <button onClick={(e) => { e.stopPropagation(); handleDelete(b.id) }} className="p-1 hover:text-white text-slate-400"><Trash2 size={12} /></button>
                      </div>
                    )}
